...
public class aic_superdriveStats extends BaseShipSystemScript {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		if (state == ShipSystemStatsScript.State.OUT) {
			...
			stats.getMaxSpeed().modifyFlat(id, 500f);
			stats.getAcceleration().modifyPercent(id, 250f * effectLevel);
			stats.getDeceleration().modifyPercent(id, -75f * effectLevel);
			stats.getTurnAcceleration().modifyFlat(id, 0f * effectLevel);
			stats.getTurnAcceleration().modifyPercent(id, -0f * effectLevel);
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (index == 0) {
			return new StatusData("reduced deceleration", false);
		} else if (index == 1) {
