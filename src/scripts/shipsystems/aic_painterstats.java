...
public class aic_painterstats extends BaseShipSystemScript {
    ...
    public static final float DAM_MULT = 1.3f;
    public static final float RANGE = 1500f;

    public static final Color TEXT_COLOR = new Color(255,55,55,255);

    public static final Color JITTER_COLOR = new Color(255, 115, 55,155);
    public static final Color JITTER_UNDER_COLOR = new Color(255, 134, 100,155);
    ...
    public void apply(MutableShipStatsAPI stats, final String id, State state, float effectLevel) {
        ShipAPI ship = null;
        if (stats.getEntity() instanceof ShipAPI && !((ShipAPI) stats.getEntity()).isFighter()) {
        ...
        if (state == State.IN && targetDataObj == null) {
            ...
            if (target != null) {
                if (target.getFluxTracker().showFloaty() ||
                    ...
                    target.getFluxTracker().showOverloadFloatyIfNeeded("Target Painted!", TEXT_COLOR, 4f, true);
                }
            }
        } else if (state == State.IDLE && targetDataObj != null) {
            Global.getCombatEngine().getCustomData().remove(targetDataKey);
            ((com.fs.starfarer.api.impl.combat.EntropyAmplifierStats.TargetData)targetDataObj).currDamMult = 1f;
            targetDataObj = null;
        }
        if (targetDataObj == null || ((com.fs.starfarer.api.impl.combat.EntropyAmplifierStats.TargetData) targetDataObj).target == null) return;
